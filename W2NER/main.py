...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        logger.info("\n{}".format(table))
        return f1, p, r, np.mean(loss_list)
...
def animator_eval(list, config, label):
    plt.figure(figsize=(10, 6))
    p, f1, r = list[0], list[1], list[2]
    plt.plot(range(config.epochs), p, label='Precise', marker='o')
    plt.plot(range(config.epochs), f1, label='F1', marker='o')
    plt.plot(range(config.epochs), r, label='Recall', marker='o')

    plt.title('Evaluation Metrics over Epochs on' + label)
    plt.xlabel('Epochs')
    ...
    plt.savefig('./figures/metric_values_' + label + '.png')
    plt.show()

def animator_loss(list, config, smooth_window=10):
    plt.figure(figsize=(10, 6))
    plt.plot(range(config.epochs), list, label='Loss', marker='o')

    plt.title('Loss over Epochs on Train Data')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
...
if __name__ == '__main__':
    ...
    train_list = [[0 for _ in range(config.epochs)] for _ in range(3)]
    test_list =  [[0 for _ in range(config.epochs)] for _ in range(3)]
    loss_list = []
    best_f1 = 0
    best_test_f1 = 0
    for i in range(config.epochs):
        logger.info("Epoch: {}".format(i))
        _, _, _, loss = trainer.train(i, train_loader)
        f1, p, r = trainer.eval(i, dev_loader)
        test_f1, test_p, test_r = trainer.eval(i, test_loader, is_test=True)
        for _ in range(3):
            train_list[0][i] = f1; train_list[1][i] = p; train_list[2][i] = r
            test_list[0][i] = test_f1; test_list[1][i] = test_p; test_list[2][i] = test_r
    ...
    animator_loss(loss_list, config)    
    animator_eval(train_list, config, label='Dev Data')
    animator_eval(test_list, config, label='Test Data')
