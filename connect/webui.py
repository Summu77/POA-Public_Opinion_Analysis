...
<path class="treePotMask" stroke="#FFF" stroke-width="8" d="M374.3,502.5c0,0-4.6,20,7.7,29.3c12.3,9.2,40.1,7.7,50.8,0s10.8-23.1,10.8-29.3"/>    
    ...
    <filter id="glow" x="-150%" y="-150%" width="200%" height="200%">
...
components.html(custom_html,width=670,height=460,scrolling=False)

def clean():

# 指定文件夹路径
    folder_path = '/mnt/data/niesen/Conect/log'

    # 遍历文件夹中的文件并删除（排除 input_text.txt）
    for filename in os.listdir(folder_path):
        if filename != 'input_text.txt':
    ...
    print(f"All files in {folder_path} (except input_text.txt) have been deleted.")


def run_command(command):  # 这一段是执行本地命令返回结果
    result = subprocess.run(command, stdout=subprocess.PIPE, text=True, shell=True)
    return result.stdout


def highlight_text(text, aspect_results):  # 这一段用于高亮显示文本
    colored_text = ""
    index_mapping = {}

    for aspect_result in aspect_results:  # 这一段是针对文本中的实体和其情感为实体标注不同的颜色
        aspect = aspect_result["Aspect"]  # 用处不大（实体文本）
        indices = aspect_result["Index"]  # 文本索引序号
        polarity = aspect_result["Polarity"]  # 情感极性

        color = "red" if polarity == 1 else "blue" if polarity == -1 else "green"  # 不同情感标不同颜色

        for index in indices:
            index_mapping[index] = color

    for i, char in enumerate(text):  # 显示颜色
        color = index_mapping.get(i, "black")
        colored_text += f'<span style="color:{color}">{char}</span>'

    return colored_text  # 返回颜色文本
...
def main():
    ...
    # 设置标题样式
    st.title("实体-立场检测")
    st.text("2023-12-13 武汉大学网安学院舆情分析小组(Ns Yxr Wcx Fjl). ")

    st.markdown(
        '<style>h1{color: #2182BF; text-align: center;} body {background-color: #303030;}</style>',
    ...
    f'font-size: 18px; '
    f'line-height: 2.0; '
    f'padding: 10px; border: 2px solid #ddd; border-radius: 10px; '
    f'color: #89C2D9; font-family: \'Arial Black\', Gadget, sans-serif;">'
    f'<b>选择测试样例或输入需要分析的语句: 👇</b><br>'
    f'</div>',
    unsafe_allow_html=True
    )

    test_cases = {
        "测试样例 1 中美关系": "武汉大学网安学院致力于维护和平，美国则经常搞破坏" ,
        "测试样例 2 中美关系": "五角大楼报告称，过去两年里，美国录得中国军队的“强制性和高风险”空中拦截显著增加；中国核武库扩张加速，极有可能已经积累了500枚核弹头，缩小了与美俄的差距。",
        "测试样例 3 中美关系": "特拉斯会见了蔡英文，并呼吁各国团结起来抵抗中国等专制政府的经济胁迫。"
    }
    selected_test_case = st.selectbox("", list(test_cases.keys()))
    input_text = st.text_area("", test_cases[selected_test_case], key="input_text",label_visibility='collapsed')

    if st.button("提交"):

        st.markdown(f"程序运行速度较慢请耐心等待...")
        with open('/mnt/data/niesen/Conect/log/input_text.txt', 'w', encoding='utf-8') as file:
            file.write(input_text)

        script_file = 'run.sh'
        try:
            subprocess.run(['./' + script_file], check=True, shell=True)
            print(f'{script_file} 已成功运行')
        except subprocess.CalledProcessError as e:
            print(f'运行 {script_file} 时出错: {e}')

        with open('/mnt/data/niesen/Conect/log/aspect_results.json', 'r', encoding='utf-8') as file:
            aspect_results = json.load(file)

        colored_text = highlight_text(input_text, aspect_results)
        ...
        st.markdown(
            ...
            f'line-height: 2.0; '
            f'padding: 10px; border: 2px solid #ddd; border-radius: 10px; '
            f'color: #5FCDD9; font-family: \'Arial Black\', Gadget, sans-serif;">'
            f'<b>分析结果:</b><br>'
            f'<span style="color: red;">(红色为正向立场实体)</span>'
            f'<span style="color: green;">(绿色为中立立场实体)</span> '
            f'<span style="color: blue;">(蓝色为负面立场实体)</span>'
            f'</div>',
            unsafe_allow_html=True
        )

        styled_text = f'<div style="font-size: larger;' \
                      f' background-color: #f0f0f0;line-height: 3.0; word-wrap: break-word; ' \
                      f'border: 1px solid #ddd; padding: 10px;">{colored_text}</div>'

        st.markdown(styled_text, unsafe_allow_html=True)
        st.text("* 模型输出结果不代表小组成员立场 ")
