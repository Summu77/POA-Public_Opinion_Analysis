...
def build_embedding_matrix(word2idx, embed_dim, dat_fname):
    if os.path.exists(dat_fname):
        ...
        fname = './glove.twitter.27B/glove.twitter.27B.' + str(embed_dim) + 'd.txt' \
            if embed_dim != 300 else './sgns.sogou.word'
...
class Tokenizer4Bert:
    def __init__(self, max_seq_len, pretrained_bert_name):
        self.tokenizer = BertTokenizer.from_pretrained("/mnt/data/niesen/ABSA-PyTorch/bert-base-chinese")
        self.max_seq_len = max_seq_len
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        for i in range(0, len(lines), 3):
            ...
            # 计算需要填充的数量
            pad_height = max(0, tokenizer.max_seq_len - idx2graph[i].shape[0])
            pad_width = max(0, tokenizer.max_seq_len - idx2graph[i].shape[1])

            # 使用 np.pad 进行填充
            dependency_graph = np.pad(
                idx2graph[i],
                ((0, pad_height), (0, pad_width)),
                mode='constant',  # 这里使用 'constant' 模式，你可以根据需求选择其他模式
                constant_values=0  # 在 'constant' 模式下，用 0 进行填充
            )
